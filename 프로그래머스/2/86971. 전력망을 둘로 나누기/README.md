# 문제풀이
구조가 트리구조라 쉽게 풀 수 있었다.  1을 최상위 노드라고 가정을 한 후 문제를 풀었을 시 <br/>
1을 기준으로 한 서브트리는n전체이며 점점 하위노드들로 내려갈수록 서브트리의 크기가 작아진다.<br/>
이때 한 전력선을 자른다는 것은 현재의 서브트리만 따로 떼어낸다고할 수 있으므로 각각의 노드들에 대한 서브트리의 개수를 구하고<br/>
|(전체개수-서브트리)-서브트리| 의 최소값을 구하면 된다.
