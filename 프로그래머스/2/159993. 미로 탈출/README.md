# 문제풀이
문제는 초기 시작 지점에서 레버까지 , 그리고 레버에서 출구까지 해당 경로의 최소값을 구하는 문제이다<br/>
단순히 bfs를 2번 돌려서 최소값을 구하면 되는 문제이며 주의해야 할 점은 o인 점 뿐만 아니라 출입구, 레버, 출구 모두 통과가 가능한 점이라는 것이다.<br/>
<br/>
우선 처음에 전체를 담색하며 시작점, 레버, 출구의 인덱스를 받는다. <br/>
해당 지점들을 시작점 -> 레버, 레버-> 출구 2구역으로 쪼개어 bfs를 돌린다. 이 때 시작점->레버까지 경로가 없으면 -1리턴, <br/>
레버 -> 출구 까지의 경로가 없으며 -1리턴을 해주며 마지막으로 각각의 결과값을 더해주면 된다.
