# 문제풀이
## 어려웠던 점
어려웠던 점은 아니고 문제가 불친절했다. 주어지는 값이 0번째부터가 아닌 1번쨰부터로 세어야 한다.<br>
즉 col, row_begin, row_end 는 각각 인덱스로 치면 +1이 된 값들이다. <br>
<br>
## 문제풀이
우선 정렬은 arras.sort내에서 comparator를 구현하여 정렬을 시킨 후<br>
반복문을 통해서 row_begin, row_end내의 행들에 대해서 각 행의 열의 합(합할 시 %(행의 값+1))을 해준 후<br>
^연산을 통해 도출한다.<br>
<br>
xor연산이란 값이 같을떄는 0 다를떄는 1을 도출한다는 특성이 있다.<br>
